...
{ 
  int instance,id,num;
  if(argint(0, &instance) < 0 || argint(1,&id) < 0 || argint(2,&num) < 0) return -1;
  
  barrier(instance,id,num);
...
sys_buffer_cond_init(void){
  return buffer_cond_init();
}

uint64
sys_cond_produce(void){
  int produced_value;
  argint(0, &produced_value);
  return cond_produce(produced_value);
}
...
sys_buffer_sem_init(void){
  return buffer_sem_init();
}

uint64 sys_sem_produce(void){
  int produced_value;
  argint(0, &produced_value);
  return sem_produce(produced_value);
}
